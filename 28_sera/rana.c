...
void avviaGranate(int* pipe_fd, int* pipe_inversa) {
    Processo rana;
    // Leggi prima le coordinate della rana
    read(pipe_inversa[0], &rana, sizeof(Processo));
    ...
    if(pid_left < 0) {
        ...
        granata(DIR_LEFT, pipe_fd, &rana); // Passa le coordinate invece della pipe
        exit(3);
    }
    ...
    if(pid_right < 0) {
        ...
        granata(DIR_RIGHT, pipe_fd, &rana); // Passa le coordinate invece della pipe
        exit(3);
    }
}
...
{
    // Non serve più leggere dalla pipe
    Processo granata;
    granata.pid = getpid();
    granata.item.id = 'o';

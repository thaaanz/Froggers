...
void *controllo(void *semafori)
{
    ...
    while(vite > 0){
        ...
        while (!reset)
        {
            ...
            stampaProiettili(astuccio, wgioco);
            ...
            case 'o': //caso granata
                ...
                for (int i = 0; i < N_GRANATE; i++)
                {
                    if (granate[i].tid == temp.tid) //cerca dove è salvata la granata nell'array
                    {
                        granate[i] = temp;
                        trovate = true;
                ...
                if (!trovate) // se non è stata trovata una granata corrispondente
                {
                    for (int i = 0; i < N_GRANATE; i++) //cerca una posizione libera
                    {
                        if (granate[i].tid == -1)
                        { 
                            granate[i] = temp;
                ...
                if (!trovate && temp.tid > 1) // se non c'espazio termino il thread
                {
                    if(pthread_cancel(temp.tid)==0){
            ...
            case '-': // caso proiettile
                ...
                {
                    if (astuccio[i].tid == temp.tid) //cerca dove è salvato il proiettile nell'array
                    {
                        astuccio[i] = temp;
                        trovati = true;
                ...
                if (!trovati)  // se non è stata trovato un proiettile corrispondente
                {
                    for (int i = 0; i < N_PROIETTILI; i++)
                    {
                        if (astuccio[i].tid == -1) //cerca una posizione libera
                        { 
                            astuccio[i] = temp;
                            trovati = true;
                            break;
                ...
                if (!trovati && temp.tid != -1)  // se non c'espazio termino il thread
                {
                    if(pthread_cancel(temp.tid) ==0 ){
                        pthread_join(temp.tid, NULL);
                    }
